...
if __name__ == '__main__':
    ...
    while True:
        ...
        if(motiontime >= 1000 and motiontime < 3000):
            ...
            print(f"State 2 (turn) mt={motiontime}")
        ...
        if(motiontime >= 3000 and motiontime < 5000):
            ...
            cmd.velocity = [0.2, 0] # -1  ~ +1
            cmd.bodyHeight = 0.1
            print(f"State 3 (walk forward) mt={motiontime}")
 
        if(motiontime >= 5000 and motiontime < 7000):
            cmd.mode = 2
            cmd.gaitType = 1
            cmd.velocity = [-0.2, 0] # -1  ~ +1
            cmd.bodyHeight = 0.1
            print(f"State 3 (walk backward) mt={motiontime}")
      
     
        if motiontime >= 7000:
            # cmd.mode = 7
            print(f"State 4 mt={motiontime}")
